...
The only canonical repository is the exact literal `docker.io/library/postgres`. A valid inspect RepoDigest is exactly `docker.io/library/postgres@sha256:<64-lowercase-hex>`. The projection validates and byte-deduplicates exact entries, then requires the distinct canonical cardinality to equal one. Zero canonical identities is `BLOCKED_PHASE_C2B_IMAGE_AUTHORITY`; more than one is `HOLD_IMAGE_IDENTITY_AMBIGUOUS`, including when the approved digest is one of them. One canonical identity coexisting with any unexpected repository identity is also `HOLD_IMAGE_IDENTITY_AMBIGUOUS`. There is no first, last, latest, sorted, or approved-preference selection.
