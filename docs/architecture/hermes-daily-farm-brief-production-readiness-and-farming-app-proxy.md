...
For the local pilot only, `ssl_mode=disable` is accepted when the database host is exactly `127.0.0.1` or `localhost`. No normalization, IPv6 loopback, wildcard bind address, LAN/Tailscale address, hostname, or host-with-port form is accepted. This exception does not authorize publishing the database port externally: only the Core HTTPS endpoint may be exposed through Tailscale. Production and other remote databases continue to require `require` or `verify-full`.
