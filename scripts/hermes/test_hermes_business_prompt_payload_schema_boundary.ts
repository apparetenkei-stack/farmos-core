...
function assertSafeBoundary(result: BoundaryResult) {
  ...
  assert.equal(result.business_prompt_payload_schema.runtime, "local_llm");
  assert.equal(
    result.business_prompt_payload_schema.payload_schema_mode,
    "dry_run_payload_schema_only",
  );
  assert.equal(
    result.business_prompt_payload_schema.configured_provider,
    "business_prompt_payload_schema",
  );
  assert.equal(
    result.business_prompt_payload_schema.schema_version,
    "hermes.business_prompt_payload.v0",
  );
  assert.equal(
    result.business_prompt_payload_schema.payload_kind,
    "business_prompt_candidate",
  );
  assert.equal(result.business_prompt_payload_schema.payload_send_allowed, false);
  assert.equal(result.business_prompt_payload_schema.source_prompt_included, false);
  assert.equal(result.business_prompt_payload_schema.sanitized_prompt_included, false);
  assert.equal(result.business_prompt_payload_schema.raw_prompt_exposed, false);
  assert.equal(
    result.business_prompt_payload_schema.sanitized_prompt_preview_exposed,
    "safe_metadata_only",
  );
  assert.equal(
    result.business_prompt_payload_schema.payload_preview_exposed,
    "safe_schema_metadata_only",
  );
  assert.equal(result.business_prompt_payload_schema.system_instruction_included, false);
  assert.equal(result.business_prompt_payload_schema.business_context_included, false);
  assert.equal(result.business_prompt_payload_schema.proposal_body_included, false);
  assert.equal(
    result.business_prompt_payload_schema.restricted_domain_data_included,
    false,
  );
  assert.equal(result.business_prompt_payload_schema.endpoint_value_exposed, false);
  assert.equal(result.business_prompt_payload_schema.model_value_exposed, false);
  assert.equal(result.business_prompt_payload_schema.credentials_required, false);
  assert.equal(result.business_prompt_payload_schema.credentials_exposed, false);
  assert.equal(result.business_prompt_payload_schema.runtime_call_allowed, false);
  assert.equal(result.business_prompt_payload_schema.request_body_created, false);
  assert.equal(result.business_prompt_payload_schema.request_body_sent, false);
  assert.equal(result.business_prompt_payload_schema.prompt_sent, false);
  assert.equal(result.business_prompt_payload_schema.response_body_exposed, false);
  assert.equal(result.business_prompt_payload_schema.selected_provider, "mock");
  assert.equal(result.business_prompt_payload_schema.fallback_provider, "mock");
  assert.equal(result.business_prompt_payload_schema.tokens_used, 0);

  assert.equal(
    result.business_prompt_payload_schema.payload_preview.source
      .raw_prompt_included,
    false,
  );
  assert.equal(
  ...
  assert.equal(
    result.business_prompt_payload_schema.payload_preview.schema
      .message_content_included,
    false,
  );
  assert.equal(
    result.business_prompt_payload_schema.payload_preview.safety
      .policy_gate_checked,
    true,
  );
  assert.equal(
  ...
  assert.equal(
    result.business_prompt_payload_schema.payload_preview.runtime
      .selected_provider,
    "mock",
  );
  assert.equal(
  ...
  assert.equal(
    result.business_prompt_payload_schema.payload_preview.transport
...
async function main() {
  const empty = await runHermesBusinessPromptPayloadSchemaBoundary({
    provider: "business_prompt_payload_schema",
    dryRun: true,
  });

  assert.equal(empty.result, "ok");
  assert.equal(empty.business_prompt_payload_schema.payload_created, false);
  assert.equal(
    empty.business_prompt_payload_schema.payload_send_decision,
    "not_configured",
  );
  assert.equal(empty.business_prompt_payload_schema.prompt_category, "unknown");
  assert.equal(empty.business_prompt_payload_schema.prompt_risk_level, "low");
  assert.equal(empty.business_prompt_payload_schema.redaction_decision, "not_required");
  assertSafeBoundary(empty);

  const operational = await runHermesBusinessPromptPayloadSchemaBoundary({
    provider: "business_prompt_payload_schema",
    dryRun: true,
    sample: "今日の作業を整理して",
  });
  ...
  assert.equal(operational.business_prompt_payload_schema.payload_created, true);
  assert.equal(
    operational.business_prompt_payload_schema.payload_send_decision,
    "dry_run_only",
  );
  assert.equal(
    operational.business_prompt_payload_schema.prompt_category,
    "operational_question",
  );
  assert.equal(operational.business_prompt_payload_schema.prompt_risk_level, "low");
  assert.equal(
    operational.business_prompt_payload_schema.redaction_decision,
    "not_required",
  );
  assertSafeBoundary(operational);

  const planning = await runHermesBusinessPromptPayloadSchemaBoundary({
    provider: "business_prompt_payload_schema",
    dryRun: true,
  ...
  assert.equal(planning.business_prompt_payload_schema.payload_created, true);
  assert.equal(
    planning.business_prompt_payload_schema.prompt_category,
    "planning_question",
  );
  assert.equal(planning.business_prompt_payload_schema.prompt_risk_level, "medium");
  assert.equal(planning.business_prompt_payload_schema.redaction_decision, "required");
  assertSafeBoundary(planning);

  const proposalRelated = await runHermesBusinessPromptPayloadSchemaBoundary({
    provider: "business_prompt_payload_schema",
    dryRun: true,
  ...
  assert.equal(
    proposalRelated.business_prompt_payload_schema.prompt_category,
    "proposal_related",
  );
  assert.equal(
    proposalRelated.business_prompt_payload_schema.prompt_risk_level,
    "high",
  );
  assert.equal(
    proposalRelated.business_prompt_payload_schema.redaction_decision,
    "required",
  );
  assert.equal(
    proposalRelated.business_prompt_payload_schema.payload_send_allowed,
    false,
  );
  assertSafeBoundary(proposalRelated);

  const restricted = await runHermesBusinessPromptPayloadSchemaBoundary({
    provider: "business_prompt_payload_schema",
    dryRun: true,
  ...
  assert.equal(restricted.business_prompt_payload_schema.payload_created, false);
  assert.equal(
    restricted.business_prompt_payload_schema.payload_send_decision,
    "blocked",
  );
  assert.equal(
    restricted.business_prompt_payload_schema.prompt_category,
    "restricted_domain",
  );
  assert.equal(
    restricted.business_prompt_payload_schema.prompt_risk_level,
    "blocked",
  );
  assert.equal(
    restricted.business_prompt_payload_schema.redaction_decision,
    "blocked",
  );
  assert.equal(
  ...
  const businessContext = await runHermesBusinessPromptPayloadSchemaBoundary({
    provider: "business_prompt_payload_schema",
    dryRun: true,
    sample: "今日の作業計画を整理して",
    businessContext: {
  ...
  assert.equal(businessContext.business_prompt_payload_schema.payload_created, false);
  assert.equal(
  ...
  const proposalBody = await runHermesBusinessPromptPayloadSchemaBoundary({
    provider: "business_prompt_payload_schema",
    dryRun: true,
  ...
  assert.equal(proposalBody.business_prompt_payload_schema.payload_created, false);
  assert.equal(
  ...
  const badProvider = await runHermesBusinessPromptPayloadSchemaBoundary({
  ...
  const nonDryRun = await runHermesBusinessPromptPayloadSchemaBoundary({
    provider: "business_prompt_payload_schema",
    dryRun: false,
    sample: "今日の作業を整理して",
  });
  ...
  const secretText = "絶対に出してはいけない固有テキストABC123";
  const noLeak = await runHermesBusinessPromptPayloadSchemaBoundary({
    provider: "business_prompt_payload_schema",
    dryRun: true,
    userMessage: secretText,
  });

  assert.equal(noLeak.result, "ok");
  assert.equal(JSON.stringify(noLeak).includes(secretText), false);
  assertSafeBoundary(noLeak);

  console.log(
    JSON.stringify(
      {
        result: "ok",
        checked: "hermes_business_prompt_payload_schema_boundary",
      },
      null,
      2,
    ),
