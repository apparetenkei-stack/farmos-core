...
async function main() {
  ...
  try {
    ...
    assert.equal(
      ...
      "local_llm_business_prompt_dry_run_contract_only",
    );
    ...
    assert.equal(
      result.switch.business_prompt_contract_status.configured_provider,
      "local_llm_business_prompt_contract",
    );
    assert.equal(
      result.switch.business_prompt_contract_status.endpoint_config_key,
      "HERMES_LOCAL_LLM_CHAT_ENDPOINT",
    );
    assert.equal(
      result.switch.business_prompt_contract_status.model_config_key,
    ...
    assert.equal(
      result.switch.business_prompt_contract_status.prompt_send_decision,
      "not_configured",
    );
    assert.equal(
      ...
      JSON.stringify(
        {
          result: "ok",
          checked:
            "hermes_llm_adapter_switch_local_business_prompt_contract_integration",
        },
        null,
        2,
      ),
