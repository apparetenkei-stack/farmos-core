...
async function main() {
  ...
  assert.equal(result.switch.requested_provider, "business_prompt_payload_schema");
  ...
  assert.equal(
    ...
    "business_prompt_payload_schema_dry_run_only",
  );
  ...
  assert.equal(
    result.switch.business_prompt_payload_schema_status.mode,
    "hermes_business_prompt_payload_schema_boundary",
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.payload_schema_mode,
    "dry_run_payload_schema_only",
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.schema_version,
    "hermes.business_prompt_payload.v0",
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.payload_kind,
    "business_prompt_candidate",
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.payload_created,
    true,
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.payload_send_allowed,
    false,
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.payload_send_decision,
    "dry_run_only",
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.payload_preview_exposed,
    "safe_schema_metadata_only",
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.source_prompt_included,
    false,
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.sanitized_prompt_included,
    false,
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.raw_prompt_exposed,
    false,
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.request_body_created,
    false,
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.request_body_sent,
    false,
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.prompt_sent,
    false,
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.selected_provider,
    "mock",
  );
  assert.equal(
    result.switch.business_prompt_payload_schema_status.fallback_provider,
    "mock",
  );
  assert.equal(result.switch.business_prompt_payload_schema_status.tokens_used, 0);

  assertSwitchBoundarySafe(result);

  const localAlias = await runHermesLlmAdapterSwitchBoundary({
    userMessage: "今日の作業計画を整理して",
    provider: "local_llm_business_prompt_payload_schema",
    dryRun: true,
  });
  ...
  assert.ok(localAlias.switch.business_prompt_payload_schema_status);
    ...
    JSON.stringify(
      {
        result: "ok",
        checked:
          "hermes_llm_adapter_switch_business_prompt_payload_schema_integration",
      },
      null,
      2,
    ),
