...
export function parseHermesDailyFarmBriefExecutionResult(value: unknown): HermesDailyFarmBriefExecutionResult | null {
  ...
  if (value.status === "completed") {
    if (decision.decision !== "generate" || !decision.should_execute_generation || value.brief_status === null || !value.snapshot_generated || !value.brief_generated || !value.scope_index_generated || !value.role_projection_generated || value.failure_code !== null || candidate === null || candidate.generated_at === null || Date.parse(candidate.generated_at) < Date.parse(value.execution_requested_at as string) || Date.parse(candidate.generated_at) > Date.parse(value.executed_at as string) || value.brief_status !== candidate.brief_status || value.visible_scope_count !== candidate.visible_scope_count || value.business_date !== candidate.business_date || value.role !== candidate.role || JSON.stringify(sourceStatus) !== JSON.stringify(candidate.source_status) || JSON.stringify(limitations) !== JSON.stringify(candidate.limitations)) return null;
  } else if (value.status === "skipped") {
    if (decision.decision === "generate" || value.snapshot_generated || value.brief_generated || value.scope_index_generated || value.role_projection_generated || value.brief_status !== null || value.visible_scope_count !== 0 || sourceStatus.length !== 0 || limitations.length !== 0 || value.failure_code !== "generation_not_authorized" || candidate !== null) return null;
  } else if (decision.decision !== "generate" || value.failure_code === null || candidate !== null || value.brief_status !== null || value.visible_scope_count !== 0 || sourceStatus.length !== 0 || limitations.length !== 0) return null;
