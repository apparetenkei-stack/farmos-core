...
async function main() {
  ...
  assert.ok(result.chat.request.normalized_user_message.length > 0);
  assert.equal(result.chat.request.requested_intent, "ask_review_queue");

  assert.ok(result.chat.safe_context);
  assert.equal(
    result.chat.safe_context.memory_context_scope,
    "hermes_memory_context_minimum",
  );
  assert.equal(
    result.chat.safe_context.review_loop_scope,
    "hermes_proposal_review_loop_minimum",
  );
  ...
  for (const message of blockedMessages) {
    const blocked = await runHermesChatInputDryRunBoundary({
      message,
      dryRun: true,
    });

    assert.equal(blocked.result, "blocked");
    assert.equal(blocked.boundary.transaction_read_only, true);
    assert.equal(blocked.boundary.writes_performed, false);
    assert.equal(blocked.boundary.commands_executed, false);
    assert.equal(blocked.boundary.llm_runtime_executed, false);
    assert.equal(blocked.boundary.external_api_called, false);
    assert.equal(blocked.boundary.proposal_apply_allowed, false);
    assert.equal(blocked.boundary.restricted_domain_data_exposed, false);
    assert.equal(blocked.chat.restricted_domain_data_exposed, false);
  }
  ...
  console.log(
    JSON.stringify(
      {
        result: "ok",
        checked: "hermes_chat_input_dry_run_boundary",
        proposal_count:
          result.chat.safety_snapshot.before.proposal_count,
        hermes_note_count:
          result.chat.safety_snapshot.before.hermes_note_count,
