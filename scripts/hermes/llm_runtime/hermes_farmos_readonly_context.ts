...
export function buildHermesCliReadonlyContextPrompt(input: {
  ...
  return [
    ...
    "日本語日付表現規則: 日本語回答ではtomorrow、today、yesterdayを使わず、それぞれ「明日」「今日」「昨日」またはcalendar_contextに基づく「7月29日」のような日本語の日付で表現してください。",
    ...
    "作業状態規則: status、started_at、開始日のいずれもcontextに明示されない限り「開始した」「完了した」「未完了」「進行中」と断定せず、recordの存在と作業状態を区別してください。記録が存在する場合は「現在参照できる記録には○○作業が含まれています」と表現してください。previewに今日の記録がない場合は「現在参照できるpreviewには今日の記録が含まれていません」と述べ、「今日作業がなかった」とは断定しないでください。",
    ...
    "回答形式: 原則として「今日の確認結果」「必要な確認」の2見出しだけを使い、各見出しの重要事項を短い箇条書きで示してください。全体で最大3項目、各項目2文以内にし、取得事実、判断不能事項、確認候補を区別してください。「推論プロセス」「最終回答」などの内部見出しは使わず、raw JSON field名は利用者が技術情報を明示的に求めた場合を除き表示しないでください。",
    "安全説明規則: 回答本文にはread-only、AIの権限、確定判断、業務実行などの独自の安全説明を生成しないでください。Slackでは共通フッターが安全境界を表示するため、本文では質問への回答だけを簡潔に示してください。",
