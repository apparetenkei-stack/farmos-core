...
export const FARM_OS_PRODUCTION_IDENTITY_QUERY_V4_ARTIFACT_PATH =
  "scripts/sql/farm_os_production_identity_readonly_v4.sql" as const;
export const FARM_OS_PRODUCTION_IDENTITY_QUERY_V4_SHA256 =
  "sha256:e83987c840cc941cf5e6dcff93d46345464db0019ea5beb5143b0222316e05ca" as const;

export const FARM_OS_PRODUCTION_IDENTITY_QUERY_V4_CANDIDATE = Object.freeze({
...
export type FarmOsProductionIdentityQueryV4SectionPlanEntry = Readonly<{
  ordinal: number;
...
export type FarmOsProductionIdentityQueryV4ArtifactVerification =
  | Readonly<{
    status: "VERIFIED";
    artifact_path: typeof FARM_OS_PRODUCTION_IDENTITY_QUERY_V4_ARTIFACT_PATH;
    sha256: typeof FARM_OS_PRODUCTION_IDENTITY_QUERY_V4_SHA256;
    raw_bytes: Uint8Array;
    section_plan: readonly FarmOsProductionIdentityQueryV4SectionPlanEntry[];
  }>
  | Readonly<{
    status: "BLOCKED";
...
function buildSectionPlan(
  sql: string,
): readonly FarmOsProductionIdentityQueryV4SectionPlanEntry[] | null {
  ...
  const entries: FarmOsProductionIdentityQueryV4SectionPlanEntry[] = [];
...
export function verifyFarmOsProductionIdentityQueryV4ArtifactBytes(
  bytes: Uint8Array | null,
): FarmOsProductionIdentityQueryV4ArtifactVerification {
  if (bytes === null) {
    return Object.freeze({
      status: "BLOCKED",
      reason: "ARTIFACT_MISSING",
      artifact_path: FARM_OS_PRODUCTION_IDENTITY_QUERY_V4_ARTIFACT_PATH,
      actual_sha256: null,
    });
  }
  const actual = sha256Bytes(bytes);
  if (actual !== FARM_OS_PRODUCTION_IDENTITY_QUERY_V4_SHA256) {
    return Object.freeze({
      status: "BLOCKED",
  ...
  return Object.freeze({
    status: "VERIFIED",
    artifact_path: FARM_OS_PRODUCTION_IDENTITY_QUERY_V4_ARTIFACT_PATH,
    sha256: FARM_OS_PRODUCTION_IDENTITY_QUERY_V4_SHA256,
    raw_bytes: bytes,
...
export function loadFarmOsProductionIdentityQueryV4Artifact():
  FarmOsProductionIdentityQueryV4ArtifactVerification {
  try {
    return verifyFarmOsProductionIdentityQueryV4ArtifactBytes(
      readFileSync(fileURLToPath(FIXED_ARTIFACT_URL)),
    );
  } catch {
    return verifyFarmOsProductionIdentityQueryV4ArtifactBytes(null);
  }
}
