import {
  ...
  type FarmOsContractParseResult, validateFarmOsReauthenticationEvidence,
...
export const FARM_OS_COMMAND_REJECTION_CODES=["PROPOSAL_NOT_APPROVED","PROPOSAL_TYPE_UNKNOWN","PROPOSAL_VERSION_UNSUPPORTED","PROPOSAL_HASH_INVALID","APPROVAL_EVIDENCE_MISSING","APPROVAL_EVIDENCE_INVALID","APPROVAL_SCOPE_MISMATCH","REAUTHENTICATION_EVIDENCE_MISSING","REAUTHENTICATION_EVIDENCE_INVALID","FINAL_CONFIRMATION_MISSING","FINAL_CONFIRMATION_SCOPE_MISMATCH","RISK_LEVEL_MISMATCH","REQUIRED_CAPABILITY_MISSING","OUTPUT_CLASS_NOT_APPROVED","COMMAND_CLASS_UNKNOWN","COMMAND_VERSION_UNSUPPORTED","COMMAND_TARGET_NOT_ALLOWED","COMMAND_SCHEMA_INVALID","TRACE_INVALID","AUDIT_CONTEXT_INVALID","ROLLBACK_CLASS_INVALID","REAUTHORIZATION_POLICY_INVALID","UNKNOWN_FIELD","DUPLICATE_FIELD","DUPLICATE_COMMAND"] as const;
export type FarmOsCommandRejectionCode=(typeof FARM_OS_COMMAND_REJECTION_CODES)[number];

export type FarmOsCommandBuildRequest={schema_version:typeof FARM_OS_COMMAND_BUILD_REQUEST_SCHEMA_VERSION;command_class:FarmOsCommandClass;command_version:1;execution_target:FarmOsCommandTarget;execution_payload:FarmOsCommandPayload;expected_business_version:number;capabilities:readonly string[];built_at:string;correlation_id:string;known_command_hashes:readonly string[]};
export type FarmOsApprovedCommand={
  ...
  execution_target:FarmOsCommandTarget;execution_payload:FarmOsCommandPayload;expected_business_version:number;
...
export type FarmOsApprovedCommandBuildResult={result:"success";command:FarmOsApprovedCommand;rejection:null}|{result:"rejected";command:null;rejection:{code:FarmOsCommandRejectionCode}};

const BUILD_KEYS=["schema_version","command_class","command_version","execution_target","execution_payload","expected_business_version","capabilities","built_at","correlation_id","known_command_hashes"] as const;
const COMMAND_KEYS=["schema_version","command_id","command_class","command_version","proposal_reference","risk_level","required_capabilities","approval_evidence","reauthorization_required","rollback_class","execution_scope","execution_target","execution_payload","expected_business_version","idempotency","audit","trace"] as const;
...
const reject=(code:FarmOsCommandRejectionCode):FarmOsApprovedCommandBuildResult=>({result:"rejected",command:null,rejection:{code}});
...
export function parseFarmOsApprovedCommand(value:unknown):FarmOsContractParseResult<FarmOsApprovedCommand>{
  ...
  if(!hasExactFarmOsKeys(value,COMMAND_KEYS))return{valid:false,value:null,blocked_reason:Object.keys(value).some((key)=>!COMMAND_KEYS.includes(key as never))?"UNKNOWN_FIELD":"COMMAND_SCHEMA_INVALID"};
  const registry=resolveFarmOsCommandClass(value.command_class);if(!registry)return{valid:false,value:null,blocked_reason:"COMMAND_CLASS_UNKNOWN"};
  ...
  if(!registry.allowed_target_systems.includes(value.execution_target as FarmOsCommandTarget))return{valid:false,value:null,blocked_reason:"COMMAND_TARGET_NOT_ALLOWED"};
  if(!payloadValid(value.execution_payload,registry.command_class,value.proposal_reference.proposal_id as string)||!Number.isSafeInteger(value.expected_business_version)||(value.expected_business_version as number)<0)return{valid:false,value:null,blocked_reason:"COMMAND_SCHEMA_INVALID"};
  ...
  const integrityMaterial={proposal_id:value.proposal_reference.proposal_id as string,proposal_type:value.proposal_reference.proposal_type as string,proposal_version:value.proposal_reference.proposal_version as number,risk_level:registry.required_risk_level,approval_requirement:registry.required_risk_level==="l2_internal_apply"?"privileged_approval" as const:"final_confirmation_and_reauthentication" as const,approved_outputs:value.approval_evidence.approved_output_classes as FarmOsApprovedProposal["approved_outputs"],approved_capabilities:value.approval_evidence.approved_capabilities as string[],source_runtime:"farmos-native-runtime" as const,trace:value.trace as FarmOsApprovedProposal["trace"],review_result:"approved" as const,review_actor:"human_reviewer" as const,review_timestamp:value.approval_evidence.review_timestamp as string,approval_id:value.approval_evidence.approval_id as string,decision:"approve" as const,reauthentication_evidence:value.approval_evidence.reauthentication_evidence as FarmOsApprovalEvidence["reauthentication_evidence"]};
  const reauthenticationError=validateFarmOsReauthenticationEvidence(integrityMaterial);
  if(reauthenticationError)return{valid:false,value:null,blocked_reason:reauthenticationError};
  const expectedProposalHash=computeFarmOsProposalIntegrityHash(integrityMaterial);
...
const draft:FarmOsApprovedCommand={schema_version:FARM_OS_APPROVED_COMMAND_SCHEMA_VERSION,command_id:commandId,command_class:registry.command_class,command_version:registry.command_version,proposal_reference:{proposal_id:proposal.proposal_id,proposal_type:proposal.proposal_type,proposal_version:proposal.proposal_version,proposal_hash:proposalHash,approval_id:proposal.approval_evidence.approval_id},risk_level:registry.required_risk_level,required_capabilities:registry.required_capabilities,approval_evidence:proposal.approval_evidence,reauthorization_required:registry.reauthorization_required,rollback_class:registry.rollback_class,execution_scope:{scope_kind:"approved_proposal_only",approved_output_class:proposal.approved_outputs[0]},execution_target:request.execution_target,execution_payload:request.execution_payload,expected_business_version:request.expected_business_version,idempotency:{command_id:commandId,command_hash:"pending",proposal_hash:proposalHash,builder_version:FARM_OS_APPROVED_COMMAND_BUILDER_VERSION,persisted:false},audit:{built_at:request.built_at,builder_id:"farm-os-approved-command-builder",review_audit_reference:proposal.audit.review_audit_reference},trace:proposal.trace};
  ...
  const parsed=parseFarmOsApprovedCommand(command);if(!parsed.valid)return reject(mapProposalReason(parsed.blocked_reason));return{result:"success",command:parsed.value,rejection:null};
}
